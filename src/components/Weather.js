...
class Weather extends Component {
  ...
  componentDidMount() {
    ...
    const weatherIcon = this.getWeatherType(
    ...
    for (let i = 0; i < 5; i++) {
      this.skycons.set(
        document.getElementById("forecast" + i),
        this.forecastIcons[i]
      );
    }
  }
  ...
  getWeatherType(weatherCode, partOfDay = "morning") {
    ...
    switch (true) {
      ...
      case x >= 801 &&
        x < 900 &&
        (partOfDay === "afternoon" || partOfDay === "morning"):
      ...
      <div className="Weather">
        <div id="mainWeatherArea">
          ...
          <div class="weatherColumn">
            ...
            <div id="description">{currentWeather.weather[0].description}</div>
            <div id="highLow">
              H {Number.parseFloat(currentWeather.main.temp_max).toFixed(0)}
              &deg; L{" "}
              {Number.parseFloat(currentWeather.main.temp_min).toFixed(0)}&deg;
            </div>
          </div>
          ...
          <div class="weatherColumn">
            <div id="info">
              ...
              <div id="time">
                as of {moment.unix(currentWeather.dt).format("h:mm A")}
              </div>
            </div>
            ...
            <div id="otherWeatherData">
              <p>
                ...
                Pressure:{" "}
                {Number.parseFloat(
                  currentWeather.main.pressure * 0.0145037738
                ).toFixed(2)}{" "}
                in
                <br />
                Wind Speed: {currentWeather.wind.speed} MPH <br />
                Sun: {moment
                  .unix(currentWeather.sys.sunrise)
                  .format("h:mm A")}{" "}
